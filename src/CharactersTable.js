...
export default function CharactersTable({ data, members = false }) {
    ...
    if (!data || !data.length) {
        ...
        characters = data.map(({ _id, name, realm, guild, hash, ilvl, media, faction, race, gender, character_class, spec, level, lastModified }) => {
            ...
            if (!members && guild) {
                guild_url = `/guild/${realm.slug}/${guild.slug}`;
        ...
        columns = [
            { title: 'Avatar', field: 'avatar', render: ({avatar}) => ((avatar) ? (<img src={avatar} alt="A" style={{width: 50, borderRadius: '50%'}}/>) : (''))},
            {
                field: '_id',
                title: 'Name',
        ...
        if (!members) {
            columns.splice(2, 0, {
                field: 'guild_name',
                title: 'Guild',
                render: rowData => <Link href={rowData.guild_url} color="secondary" underline="hover">{rowData.guild_name}</Link>
            })
