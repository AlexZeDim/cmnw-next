...
Любые команды в DMA модуле содержат в себе два аргумента **ITEM (предмет)** и **REALM(S) сервер(а)**. Независимо от того, пользуетесь вы сайтом или дискорд ботом.

Сперва-наперво: **ITEM (предметы)**. Для запроса, вы можете использовать ID предмета, его название или короткие быстро запоминающиеся названия - ***тикеры***.
 ...
 результаты поиска могут несколько отличаться от желаемого. В таком случае используйте тикеры или ID предмета (ID предмета самый точный способ запроса). Большинство тикеров, очевидны по их названию,
...
> Возможно вы не знаете этого, но команда `>ITEM` возвращает все наиболее подходящие предметы (т.е наивысшее значение в колонке score) по запросу `>GROUPITEMS`.
...
Давайте-ка запросим алхимические зелья, что бы получше понять:
